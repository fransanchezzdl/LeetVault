...
export function ApiKeyDialog({ open, initialValue, onClose, onSaved }: Props) {
  ...
  useEffect(() => {
    ...
    void window.lv.settings.get(VOICE_ID_KEY).then((saved) => {
      if (
        saved &&
        (saved === SYSTEM_VOICE_ID || ORPHEUS_VOICES.some((v) => v.id === saved))
      ) {
        setVoiceId(saved);
      }
    });
      ...
      <div className="glass-card-dim w-[460px] max-w-[92%] p-5">
        ...
        <div className="mt-4 border-t border-glass-stroke/60 pt-4">
          ...
          <p className="mb-2 text-[11px] leading-relaxed text-fg/[0.68]">
            Groq voices (Orpheus) sound natural but need internet, your API key, and a
            one-time terms acceptance in the Groq console. The system voice is robotic but
            works offline and has no usage limits. If a Groq voice fails mid-interview,
            playback falls back to the system voice.
          </p>
          <button
            type="button"
            onClick={() =>
            ...
            className="mb-2 inline-flex items-center gap-1.5 text-[11px] text-brand-400 hover:underline"
          >
            Accept the Orpheus terms in the Groq console <ExternalLink className="h-3 w-3" />
          </button>
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <option value={SYSTEM_VOICE_ID}>System (offline, robotic)</option>
            </select>
            <button
              type="button"
              onClick={preview}
          ...
          {previewError ? (
            <p className="mt-2 text-[11px] text-diff-hard">{previewError}</p>
          ) : null}
        </div>
